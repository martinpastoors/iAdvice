...
# -----------------------------------------------------------------------------------------------
# Paper 02 Scale-jump analysis of historic retrospective patterns.Rmd
#
...
*Figure 7: Comparisons of stock size (SSB) in assessments where stock definitions have changed. Blue-green is old stock definition, red is new stock definition*
...
# plot number of scale jumps by decade
...
t <-
  ...
  group_by(variable, decade, scale_jump) %>% 
  summarize(n = n()) %>% 
...
p1 <-
  ggplot(t, aes(x=reorder(decade, -decade), y=prop)) +
  theme_publication() +
  theme(
    panel.spacing    = unit(0.1, "lines"),
    panel.grid.major.x = element_line(colour = "gray80"),
  ...
  geom_bar(stat="identity") +
  scale_y_continuous("Proportion scale jumps", breaks=pretty_breaks(n=4)) +
  labs(x="") +
  coord_flip()
  
p2 <-
  t %>% 
  ggplot(aes(x=reorder(decade, -decade), y=nassess)) +
  theme_publication() +
  theme(
    panel.spacing      = unit(0.1, "lines"),
    panel.grid.major.x = element_line(colour = "gray80"),
...
# scaling of the two axes
# ylim.prim <- c(0, 0.5)     # proportions
# ylim.sec  <- c(0, 90)     # number of assessments
...
# t %>%   
#   ggplot(aes(x=reorder(decade, -decade), y=prop)) +
#   theme_publication() +
#   theme(
#     panel.spacing    = unit(0.1, "lines"),
#     panel.grid.major.x = element_line(colour = "gray80"),
...
#   
#   geom_bar(stat="identity") +
#   geom_line(data=t2, aes(x= reorder(decade, -decade), y = a + nassess * b), color = "red", size=1) +
#   scale_y_continuous("Proportion scale jumps", 
#                      sec.axis = sec_axis(~ (. - a)/b, 
#                                          name = "N Assessments", 
#                                          breaks=pretty_breaks(n=4))) +
...
*Figure 6: Proportion of scale jumps in stocksize by decade, using 5 year means and a threshold of 10%. The red line indicates the number of assessments included in the comparison (right hand y-axis)*
...
p1 <-
  ...
  ggplot(aes(x=reorder(assessmentmodel, -rank), y=prop)) +
  theme_publication() +
  theme(
    panel.spacing    = unit(0.1, "lines"),
    panel.grid.major.x = element_line(colour = "gray80"),
...
p2 <-
  ...
  ggplot(aes(x=reorder(assessmentmodel, -rank), y=nassess)) +
  theme_publication() +
  theme(
    panel.spacing    = unit(0.1, "lines"),
    panel.grid.major.x = element_line(colour = "gray80"),
  ...
  labs(x="") +
  scale_y_continuous("N assessments", breaks=pretty_breaks(n=4)) +
  coord_flip() 
