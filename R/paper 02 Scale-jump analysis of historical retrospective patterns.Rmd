...
distinct(iAssess, stockkeylabelold, assessmentyear, purpose, benchmark) %>% filter(grepl("ple-n", stockkeylabelold)) %>% View()
...
d.assessments <-
  ...
  mutate(assessmentmodelchange = ifelse(assessmentmodel != lag(assessmentmodel), 1, 0),
         assessmentmodelchange = ifelse(is.na(assessmentmodelchange), 0, assessmentmodelchange)) %>% 
  
  group_by(stock) %>% 
  arrange(stock, assessmentyear) %>% 
  mutate(stockdefinitionchange = ifelse(stockkeylabelold != lag(stockkeylabelold), 1, 0),
...
# Calculations of scale-differences and scale jumps ==========================================
...
fig_nums(name    = "scalejumps", level = 1, display = FALSE, 
         caption = "Retrospective scale difference (sigma) for 9 species derived from historical assessment records. Results from pairwise comparisons over periods of 8 years. Large dots  refer to years when the scale difference is larger than 10%. Numbers at the bottom left of each panel refer to the number of assessments with a scale difference larger than 10% relative to the total number of assessments available for that stock. Colours indicate the decades. Black triangles and dashed lines indicate benchmark assessment years. Red triangles and dashed lines indicate years when assessment models were changed compared to the year before")
