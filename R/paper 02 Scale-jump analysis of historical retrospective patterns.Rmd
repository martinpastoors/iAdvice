...
pairwise_comparisons <- function(removelastyears=0, numberofyears=10, threshold=0.10) {
  ...
  d.pcm <-
    ...
    mutate(phi            = (prior-last)/last,
           ...
           scale_jump     = ifelse(phi_abs  > threshold, TRUE, NA),
           scale_jump     = ifelse(phi_abs <= threshold, FALSE, scale_jump)) %>% 
    group_by(stock, variable) %>% 
    mutate(scale_jump     = ifelse(row_number() == 1, NA, scale_jump),
           removelastyears= removelastyears, 
...
tmp <- pairwise_comparisons(removelastyears = 0, numberofyears=5, threshold = 0.1)
...
res <- pairwise_comparisons(removelastyears = 0, numberofyears=10, threshold=0.1)
