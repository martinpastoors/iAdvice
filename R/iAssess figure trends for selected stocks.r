...
# iAssess %>% filter(assessmentyear == 2019, stockkeylabelold=="her-67bc") %>% View()
...
d <-
  ...
  filter(grepl("her-67bc|her-3a22|her-vian|her-47d3", stockkeylabelold) ) %>%
  # filter(grepl("her-irls|her-67bc|her-3a22|her-47d3|her-nirs", stockkeylabelold) ) %>%
  filter(assessmentyear   %in%  2014:2019) %>% 
  filter(year             >   1992) %>% 
...
d %>% 
  ...
  geom_line(aes(colour = stockkeylabelold) ) +
