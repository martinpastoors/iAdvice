...
my_pres <- my_pres %>% 
  ...
  ph_with(value = external_img(file.path(figuresdir, "neafc-area.png"),
                               width=4, height=5),
          # location = ph_location_label(ph_label="Content right")) %>% 
          location = ph_location(left = 8.5, top = 2),
          use_loc_size = FALSE) %>% 
  ph_with(value = external_img(file.path(figuresdir, "coastal states.png"), 
                               width=7, height=5), 
        # location = ph_location_label(ph_label="Content right")) 
          location = ph_location(left = 0.8, top = 2.75),
          use_loc_size = FALSE)
...
#   filter(year           < max(assessmentyear)) %>% 
...
#   
#   group_by(fisheriesguild, speciesfaocode, stockkeylabelold, year) %>% 
...
# iAdvice %>% 
#   filter(stockkeylabelold=="cap-bars") %>%
#   rename(year=assessmentyear) %>% 
#   mutate(fisheriesguild="pelagic") %>% 
#   group_by(fisheriesguild, speciesfaocode, stockkeylabelold, year) %>% 
#   summarize(catches  = sum(catches, na.rm=TRUE),
#             landings = sum(landings, na.rm=TRUE)) %>% 
#   View()
...
d <-
  ...
  # filter(speciesfaocode=="whb") %>% View()

  summarize(catches  = sum(catches, na.rm=TRUE),
            landings = sum(landings, na.rm=TRUE)) %>% 
  
  # add barents sea capalin; it is not in iAssess
  bind_rows(
    iAdvice %>% 
      filter(stockkeylabelold=="cap-bars") %>%
...
t <-
  ...
  # (catches  = sum(catches, na.rm=TRUE)/1000000,
  #           landings = sum(landings, na.rm=TRUE)/1000000) %>% 
  group_by(year) %>% 
...
myPelColors                <- RColorBrewer::brewer.pal(12, "Paired")[1:9]
