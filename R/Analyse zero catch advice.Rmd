...
# -----------------------------------------------------------------------------------------------
# Analyse zero catch advice.Rmd
#
# 26/02/2020 adapted from Analyse changes in pelagic stocks
# -----------------------------------------------------------------------------------------------
...
adv %>% 
  filter(purpose == "advice") %>% 
  filter(advisedlandingsmax == 0 | advisedcatchmax == 0) %>%  
  filter(tacyear <= 2018) %>% 
  filter(!is.na(tal)) %>% 
  dplyr::select(speciesfaocode, stockkeylabelold, assessmentyear, tacyear, advisedlandingsmax, tal, landings, catches) %>%
  ungroup() %>% 
  mutate(catch     = ifelse(!is.na(catches), catches, landings)) %>% 
  mutate(zerotac   = ifelse(tal   == 0, 1, 0)) %>% 
  mutate(zerocatch = ifelse(catch == 0, 1, 0)) %>% 
  # View()
  group_by(tacyear) %>% 
  summarize(nzeroadvice = n(),
            nzerotac    = sum(zerotac, na.rm=TRUE),
            nzerocatch  = sum(zerocatch, na.rm=TRUE)) %>% 
  gather(key="variable", value="data", nzeroadvice:nzerocatch) %>% 
  
  ggplot(aes(x=tacyear)) +
  theme_publication() +
  theme(legend.title = element_blank()) +
  
  geom_line(aes(y=data, group=variable, colour=factor(variable))) +
  geom_point(aes(y=data, group=variable, colour=factor(variable))) +
...
adv %>% 
  ...
  filter(tacyear <= 2018) %>% 
  filter(!is.na(tal)) %>% 
  dplyr::select(speciesfaocode, stockkeylabelold, stockkeylabelnew, assessmentyear, tacyear, advisedlandingsmax, tal, landings, catches) %>%
  ungroup() %>% 
  mutate(catch     = ifelse(!is.na(catches), catches, landings)) %>% 
  mutate(zerotac   = ifelse(tal   == 0, 1, 0)) %>% 
  mutate(zerocatch = ifelse(catch == 0, 1, 0)) %>% 
  
  write.csv(file="zerocatch.csv", row.names = FALSE)
